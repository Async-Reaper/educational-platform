const fs = require('fs');
const jsonServer = require('json-server');
const path = require('path');

const server = jsonServer.create();
const router = jsonServer.router(path.resolve(__dirname, 'db.json'));

server.use(jsonServer.defaults({}));
server.use(jsonServer.bodyParser);

server.use(async (req, res, next) => {
   await new Promise((res) => {
      setTimeout(res, 800);
   });
   next();
});

server.get('/courses/get_courses/programming', (req, res) => {
   try {
      const db = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'db.json'), 'UTF-8'));

      res.json(db.course[0]);
   } catch (e) {
      console.log(e);
      return res.status(500).json({ message: e.message });
   }
});

server.get('/courses/get_courses/web_design', (req, res) => {
   try {
      const db = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'db.json'), 'UTF-8'));

      res.json(db.course[1]);
   } catch (e) {
      console.log(e);
      return res.status(500).json({ message: e.message });
   }
});

server.get('/courses/get_courses/secure_information', (req, res) => {
   try {
      const db = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'db.json'), 'UTF-8'));

      res.json(db.course[2]);
   } catch (e) {
      console.log(e);
      return res.status(500).json({ message: e.message });
   }
});

server.get('/courses/get_courses/3d_modeling', (req, res) => {
   try {
      const db = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'db.json'), 'UTF-8'));

      res.json(db.course[3]);
   } catch (e) {
      console.log(e);
      return res.status(500).json({ message: e.message });
   }
});

// eslint-disable-next-line consistent-return
server.post('/courses/upload__resource/', (req, res) => {
   try {
      const {
         title, file, creator, resourceName, resourceType,
      } = req.body;

      const db = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'db.json'), 'UTF-8'));

      const storage = getStorage();
      const mountainsRef = ref(storage, 'mountains.jpg');
      const mountainImagesRef = ref(storage, 'images/mountains.jpg');
   } catch (e) {
      console.log(e);
      return res.status(500).json({ message: 'Course not found' });
   }
});

server.post('/user/login', (req, res) => {
   try {
      const { phone } = req.body;
      const db = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'db.json'), 'UTF-8'));
      const { users = [] } = db;

      const userFromBd = users.find(
         (user) => user.phone === phone,
      );

      if (userFromBd) {
         return res.json(userFromBd);
      }
      return res.json({ message: 'Пользователь не найден' });
   } catch (e) {
      console.log(e);
      return res.status(500).json({ message: 'Course not found' });
   }
});

server.post('/user/signup', (req, res) => {
   try {
      const data = req.body;
      const db = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'db.json'), 'UTF-8'));
      const { users = [] } = db;

      const userFromBd = users.find(
         (user) => user.phone === data.phone,
      );

      if (userFromBd) {
         return res.json(userFromBd);
      }
      if (!userFromBd) {
         db.users.push(data);
         const dataS = JSON.stringify(db);
         fs.writeFileSync(path.resolve(__dirname, 'db.json'), dataS);

         return res.json('Регистрация прошла успешно');
      }
   } catch (e) {
      console.log(e);
      return res.status(500).json({ message: 'Course not found' });
   }
});

server.post('/course/upload__resource', (req, res) => {
   try {
      const data = req.body;
      const uploadFiles = {
         title: data.title,

      }
      const db = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'db.json'), 'UTF-8'));
      const { course = [] } = db;
      course.map((cour) => {
         if (data.nameResource === cour.title) {
            if (data.typeResource === 'programming') {
               db.course.push(data);
               const dataS = JSON.stringify(db);
               fs.writeFileSync(path.resolve(__dirname, 'db.json'), dataS);
            }

            return res.json(cour.files);
         }
      });
   } catch (e) {

   }
});

server.use(router);

// запуск сервера
server.listen(4000, () => {
   console.log('server is running on 4000 port');
});
